...
def summarize(folder: Path):
            ...
            for repeat in (1, 2):
                ...
                if raw_path.exists():
                    ...
                    if candidate.get("finish_reason") == "STOP":
                        ...
                        row["worse_than_zero"] = row["ecl_regret"] > row["zero_regret"]
                        row["regret_to_zero_ratio"] = (
                            row["ecl_regret"] / row["zero_regret"]
                            if row["zero_regret"]
                            else None
                        )
                        if predictions:
        ...
        for scenario in ("baseline", "adverse", "benign"):
            ...
            if len(values) == 2:
                ...
                repeat_comparisons.append(
                    {
                        "scenario": scenario,
                        "mae_ratio": max(errors) / min(errors)
                        if min(errors) > 0
                        else None,
                        "regret_ratio": max(values) / min(values)
                        if min(values) > 0
                        else None,
                    }
                )
        summary.append(
            {
                ...
                "planned": 6,
                "rankable": complete,
                "worse_than_zero_runs": sum(r["worse_than_zero"] for r in measured),
    ...
    lines.extend(
        [
            ...
            (
                ...
                "Gemini 2.5 Flash-Lite returned HTTP 404 in a preliminary attempt; its bulky inputs/truth archive was removed during review."
            ),
            "",
            (
    ...
    for result in summary:
        ...
        if model in matched:
            findings.append(
                f"**{model} was worse than predicting zero for every pool on {result['worse_than_zero_runs']} of {result['measured']} measured runs**, on the primary squared-error metric."
            )
            findings.append("")
            for row in groups[model]:
                if row.get("worse_than_zero"):
                    ratio = number(row["regret_to_zero_ratio"], ".2f")
                    findings.append(
                        f"- {row['scenario'].title()} repeat {row['repeat']}: regret {row['ecl_regret']:.6g}, versus zero-answer regret {row['zero_regret']:.6g} ({ratio} times as large)."
                    )
            findings.append("")
            for pair in result["repeat_comparisons"]:
                findings.append(
                    f"On byte-identical {pair['scenario']} inputs, the two repeats differed by {number(pair['mae_ratio'], '.2f')} times in mean absolute loss-rate error and {number(pair['regret_ratio'], '.2f')} times in squared-error regret. These ratios describe different metrics and must not be conflated."
                )
                findings.append("")
            findings.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "folder", nargs="?", default="pilots/cecl-gemini-september-2026"
    )
